...
build=(0 1 1)               # Array to control execution of builder scripts (0 = off, 1 = on)

# Common Settings: Basic Configuration Options Used Across Scripts
mode='RMW'                  # Operation mode: VMAX/PMIN/RMW/ALL
...
if [ "$data_source" = "MERRA2" ]; then
    text_report_name="${mode}LIN_${var_num}c.txt"        # text report (saved under workdir/text_report)
    model_name="LIN_${windowsize_x}w_${var_num}c"        # Model name assigned to each experiemnts
    expName="000"
elif [ "$data_source" = "WRF" ]; then
