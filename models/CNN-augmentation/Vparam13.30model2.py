    ...
    for dense_layer in dense_layers:
        for layer_size in layer_sizes:
            for conv_layer in conv_layers:
                NAME =root+'model/'+'Vparam13.30testmodel2'
...
X = np.load(root+'data/train13x.30x30.npy')
X=np.transpose(X, (0, 2, 3, 1))
y = np.load(root+'data/train13y.30x30.npy')[:,0]
number_channels=X.shape[3]
...
for i in range(y.shape[1]):
        ...
        '''
x_train=resize_preprocess(x_train, 128,128, 'lanczos5')
DENSE_LAYER = [2]
LAYER_SIZES = [32]
CONV_LAYERS = [3]
