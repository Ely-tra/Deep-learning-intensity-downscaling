    ...
    for dense_layer in dense_layers:
        for layer_size in layer_sizes:
            for conv_layer in conv_layers:
                NAME =root+'model/'+'3param13testmodel2'
                ...
                outputs = layers.Dense(3,activation=activ ,name="my_dense")(x)
                ...
                model.compile(loss=loss,optimizer="adam",metrics=[mae_for_output(i) for i in range(3)] + [rmse_for_output(i) for i in range(3)])
...
y = np.load(root+'data/train13y.npy')[:]
number_channels=X.shape[3]
