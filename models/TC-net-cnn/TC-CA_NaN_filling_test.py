...
def fill4(array, ref, fix):
    ...
    Uses the first referenced channel (index 0 of ref) for boundary extraction and
    the reference channels (array[ref]) for vector (wind) calculation.
    Only channels in fix are modified.
    
    Parameters:
    - array: numpy array of shape (n_channels, H, W)
    - ref: list/tuple of indices for reference channels (e.g., [0, 1])
    - fix: list/tuple of indices for channels that need fixing (e.g., [0, 1, 2, 3])
    
    Returns:
    - array: numpy array with some NaN values filled.
    """
...
def fill_nan(array, ref, fix):
    ...
    Returns:
    - array: numpy array with NaN values filled.
    """
    hold1 = 0
    while np.sum(np.isnan(array[fix[0], 1:-1, 1:-1])) > 0:
        ...
        if hold1 == 300:
            break
    array = np.pad(array, ((0, 0), (1, 1), (1, 1)), mode='constant', constant_values=np.nan)
    ...
    for i in range(len(xa)):
        ...
        if np.isnan(np.sum(xa[i])):
            ...
            for ref_channels, fix_channels in channel_mapping.items():
                ...
                # Fix the NaNs using the fill_nan function, passing ref and fix arrays
                fixed_group = fill_nan(group_data, ref_channels, fix_channels)
